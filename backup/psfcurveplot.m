...
for i=1:length(obj.guihandles.PSFdata.data)
    FWHMset=[FWHMset;obj.guihandles.PSFdata.data{i,1}.FWHMset];
end
...
FWHM_z=round(FWHM(reshape(Iprof_z,[dimz,1]),stepsize*dimz,intp_z),2);
...
plot(coor_x,Iprof_x);
plot(coor_y,Iprof_y);
plot(coor_z,reshape(Iprof_z,[dimz,1]));
